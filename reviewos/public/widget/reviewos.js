"use strict";(()=>{function C(t){let e=t,a=new Set;return{getState:()=>e,setState(r){e={...e,...r},a.forEach(o=>o())},subscribe(r){return a.add(r),()=>a.delete(r)}}}async function b(t){let e=await fetch(t);if(!e.ok)throw new Error(`request failed: ${e.status}`);return e.json()}async function j(t,e){return(await b(`${t}/api/products/${encodeURIComponent(e)}`)).product}async function O(t,e){return(await b(`${t}/api/reviews/summary?product=${encodeURIComponent(e)}`)).summary}async function D(t,e){return(await b(`${t}/api/attributes?category=${encodeURIComponent(e)}`)).attributes.filter(r=>r.display)}async function L(t,e){let a=new URLSearchParams;a.set("product",e.productSlug),a.set("sort",e.sort),a.set("page",String(e.page)),a.set("pageSize",String(e.pageSize)),e.rating&&a.set("rating",String(e.rating));for(let[o,u]of Object.entries(e.attrFilters))a.set(o,u);return await b(`${t}/api/reviews?${a.toString()}`)}async function H(t,e,a){let r=new URLSearchParams;r.set("product",e);for(let[u,d]of Object.entries(a))r.set(u,d);return(await b(`${t}/api/ai/summary?${r.toString()}`)).summary}async function N(t,e){return(await b(`${t}/api/marketplace?product=${encodeURIComponent(e)}`)).stats}async function q(t,e){let a=await fetch(`${t}/api/reviews/${encodeURIComponent(e)}/helpful`,{method:"POST"});if(!a.ok)throw new Error(`request failed: ${a.status}`);return a.json()}async function U(t,e){var r;let a=await fetch(`${t}/api/reviews`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)});if(!a.ok){let o=await a.json().catch(()=>({}));throw new Error((r=o.error)!=null?r:`request failed: ${a.status}`)}return a.json()}function l(t){return String(t!=null?t:"").replace(/[&<>"']/g,e=>{switch(e){case"&":return"&amp;";case"<":return"&lt;";case">":return"&gt;";case'"':return"&quot;";default:return"&#39;"}})}function f(t,e=5){let a="";for(let r=1;r<=e;r++)a+=`<span class="rvos-star ${r<=t?"rvos-star--filled":""}">&#9733;</span>`;return a}function z(t){let e=new Date(t);return Number.isNaN(e.getTime())?"":e.toLocaleDateString(void 0,{year:"numeric",month:"short",day:"numeric"})}function G(t){if(t.aiSummaryLoading)return'<div class="rvos-ai-summary rvos-ai-summary--loading">Generating AI summary\u2026</div>';let e=t.aiSummary;if(!e)return"";let a=Object.keys(t.attrFilters).length,r=a>0?`AI summary of ${e.reviewCount} review${e.reviewCount===1?"":"s"} matching ${a} filter${a===1?"":"s"}`:`AI summary of ${e.reviewCount} review${e.reviewCount===1?"":"s"}`,o=e.pros.map(d=>`<li class="rvos-ai-summary__pro">${l(d)}</li>`).join(""),u=e.cons.map(d=>`<li class="rvos-ai-summary__con">${l(d)}</li>`).join("");return`
  ...
  `}function B(t){let e=t.summary;return e?`
  ...
  `:""}function V(t){let e=t.marketplaceStats;return!e||e.length===0?"":`
    <div class="rvos-trust-badges">
      <div class="rvos-trust-badges__row">${e.map(r=>{let o=l(r.source.name.trim().charAt(0).toUpperCase()||"?"),u=r.source.logoUrl?`<img class="rvos-trust-badge__logo" src="${l(r.source.logoUrl)}" alt="${l(r.source.name)}" onerror="this.style.display='none';this.nextElementSibling.style.display=''" /><div class="rvos-trust-badge__fallback" style="display:none">${o}</div>`:`<div class="rvos-trust-badge__fallback">${o}</div>`;return`
  ...
  `}function Q(t){let e=t.summary;if(!e||e.count===0)return"";let a=[5,4,3,2,1].map(o=>{var v;let u=(v=e.byStar[String(o)])!=null?v:0,d=e.count>0?Math.round(u/e.count*100):0;return`
      ...
      `}).join(""),a=Object.entries(t.attrFilters);return`<div class="rvos-filters">${a.length>0?`<div class="rvos-active-filters">
        ...
        </div>`:""}${e}</div>`}var lt=8;function S(t){let e=[];for(let a of t.galleryReviews)for(let r of a.media)r.type==="image"&&e.push({url:r.url,review:a});return e}function K(t){let e=S(t);if(e.length===0)return"";let a=e.slice(0,lt),r=e.length-a.length,o=a.map((d,s)=>`
  ...
  `}var ct=[{value:"recent",label:"Most recent"},{value:"helpful",label:"Most helpful"},{value:"rating_desc",label:"Highest rating"},{value:"rating_asc",label:"Lowest rating"}];function ut(t,e){let a=[t.verifiedBuyer?'<span class="rvos-badge">Verified buyer</span>':"",t.verifiedMarketplace?'<span class="rvos-badge rvos-badge--marketplace">Verified marketplace</span>':""].join(""),r=t.media.length?`<div class="rvos-card__media">${t.media.map(u=>`<img class="rvos-card__thumb" src="${l(u.url)}" alt="review media" loading="lazy" />`).join("")}</div>`:"",o=t.merchantReply?`<div class="rvos-card__reply">
    ...
    <article class="rvos-card" data-review-id="${l(t.id)}">
      <div class="rvos-card__head">
        ...
        ${a}
        <div class="rvos-card__date">${z(t.createdAt)}</div>
      </div>
  ...
  `}function Z(t){let e=ct.map(d=>`<option value="${d.value}" ${t.sort===d.value?"selected":""}>${d.label}</option>`).join(""),a=`
  ...
  `}var tt=new Set(["rating","sort","page"]);function et(t){let e=new URLSearchParams(window.location.search),a={},r=new Set(t.map(s=>s.key));for(let[s,v]of e.entries())r.has(s)&&!tt.has(s)&&(a[s]=v);let o=e.get("rating"),u=o?Number(o):null,d=e.get("sort")||"recent";return{attrFilters:a,rating:u,sort:d}}function rt(t,e,a){let r=new URLSearchParams(window.location.search);for(let d of Array.from(r.keys()))(tt.has(d)||t[d]!==void 0)&&r.delete(d);for(let[d,s]of Object.entries(t))r.set(d,s);e&&r.set("rating",String(e)),a!=="recent"&&r.set("sort",a);let o=r.toString(),u=`${window.location.pathname}${o?`?${o}`:""}${window.location.hash}`;window.history.replaceState(null,"",u)}var dt=["ai-summary","summary","trust-badges","distribution","filters","ugc-gallery","feed","write"],at=5;function st(t){var P,A;let e=t.dataset.product;if(!e){console.error("[reviewos] missing data-product attribute");return}let a=e,r=((P=t.dataset.api)!=null?P:"").replace(/\/$/,""),o=((A=t.dataset.blocks)!=null?A:dt.join(",")).split(",").map(i=>i.trim()).filter(Boolean),u=new Set(o),s=C({apiBase:r,productSlug:a,blocks:u,loading:!0,error:null,product:null,summary:null,attributeDefs:[],aiSummary:null,aiSummaryLoading:!1,marketplaceStats:[],lightboxIndex:null,lightboxReturnIndex:null,galleryReviews:[],reviews:[],total:0,page:1,pageSize:at,reviewsLoading:!1,votedIds:{},ratingFilter:null,attrFilters:{},sort:"recent",writeOpen:!1,writeRating:0,writeSubmitting:!1,writeSuccess:!1,writeError:null});t.innerHTML='<div class="rvos-widget"></div>';let v=t.querySelector(".rvos-widget");function F(){let i=s.getState();if(i.loading){v.innerHTML='<div class="rvos-loading">Loading reviews\u2026</div>';return}if(i.error){v.innerHTML=`<div class="rvos-error">Couldn't load reviews. Please try again later.</div>`;return}let n=[];if(i.blocks.has("ai-summary")&&n.push(G(i)),i.blocks.has("summary")&&n.push(B(i)),i.blocks.has("trust-badges")&&n.push(V(i)),i.blocks.has("distribution")&&n.push(Q(i)),i.blocks.has("filters")&&n.push(J(i)),i.blocks.has("ugc-gallery")&&n.push(K(i)),i.blocks.has("feed")&&n.push(Z(i)),v.innerHTML=n.join("")+(i.blocks.has("write")?X(i):"")+Y(i),y!==null){let g=y;y=null;let c=v.querySelector(`[data-photo-index="${g}"]`);c==null||c.focus()}}let y=null;s.subscribe(F);let $=0;async function _(i){let n=s.getState(),g=++$;s.setState({reviewsLoading:!0});try{let c=await L(r,{productSlug:a,rating:n.ratingFilter,attrFilters:n.attrFilters,sort:n.sort,page:i?1:n.page,pageSize:at});if(g!==$)return;let m=i?[]:s.getState().reviews;s.setState({reviews:[...m,...c.reviews],total:c.total,page:c.page,reviewsLoading:!1})}catch(c){if(g!==$)return;s.setState({reviewsLoading:!1,error:"reviews_failed"})}}let x=0;async function E(){if(!s.getState().blocks.has("ugc-gallery"))return;let i=s.getState(),n=++x;try{let g=await L(r,{productSlug:a,rating:i.ratingFilter,attrFilters:i.attrFilters,sort:i.sort,page:1,pageSize:200});if(n!==x)return;s.setState({galleryReviews:g.reviews})}catch(g){if(n!==x)return;s.setState({galleryReviews:[]})}}let R=0;async function M(){if(!s.getState().blocks.has("ai-summary"))return;let i=++R;s.setState({aiSummaryLoading:!0});try{let n=await H(r,a,s.getState().attrFilters);if(i!==R)return;s.setState({aiSummary:n,aiSummaryLoading:!1})}catch(n){if(i!==R)return;s.setState({aiSummary:null,aiSummaryLoading:!1})}}async function w(){s.getState().lightboxIndex!==null&&s.setState({lightboxIndex:null,lightboxReturnIndex:null}),rt(s.getState().attrFilters,s.getState().ratingFilter,s.getState().sort),await Promise.all([_(!0),M(),E()])}async function nt(){try{let i=await j(r,a),[n,g]=await Promise.all([O(r,a),D(r,i.category)]),c=et(g);s.setState({product:i,summary:n,attributeDefs:g,ratingFilter:c.rating,attrFilters:c.attrFilters,sort:c.sort,loading:!1}),s.getState().blocks.has("trust-badges")&&N(r,a).then(m=>s.setState({marketplaceStats:m})).catch(()=>s.setState({marketplaceStats:[]})),await Promise.all([_(!0),M(),E()])}catch(i){console.error("[reviewos] init failed",i),s.setState({loading:!1,error:"init_failed"})}}v.addEventListener("click",async i=>{let n=i.target.closest("[data-action]");if(!n)return;let g=n.dataset.action;if(g==="filter-rating"){let c=n.dataset.rating,m=c?Number(c):null,p=s.getState().ratingFilter;s.setState({ratingFilter:p===m?null:m}),await w();return}if(g==="toggle-filter"){let c=n.dataset.key,m=n.dataset.value,p={...s.getState().attrFilters};p[c]===m?delete p[c]:p[c]=m,s.setState({attrFilters:p}),await w();return}if(g==="clear-filters"){s.setState({attrFilters:{},ratingFilter:null}),await w();return}if(g==="load-more"){if(s.getState().reviewsLoading)return;s.setState({page:s.getState().page+1}),await _(!1);return}if(g==="vote-helpful"){let c=n.dataset.reviewId,m=s.getState();if(m.votedIds[c])return;s.setState({votedIds:{...m.votedIds,[c]:!0},reviews:m.reviews.map(p=>p.id===c?{...p,helpfulCount:p.helpfulCount+1}:p)});try{await q(r,c)}catch(p){}return}if(g==="open-write"){s.setState({writeOpen:!0,writeSuccess:!1,writeError:null,writeRating:0});return}if(g==="close-write"){s.setState({writeOpen:!1});return}if(g==="set-write-rating"){s.setState({writeRating:Number(n.dataset.value)});return}if(g==="open-lightbox"){let c=Number(n.dataset.photoIndex);s.setState({lightboxIndex:c,lightboxReturnIndex:c});return}if(g==="close-lightbox"){y=s.getState().lightboxReturnIndex,s.setState({lightboxIndex:null,lightboxReturnIndex:null});return}if(g==="lightbox-prev"){let c=s.getState().lightboxIndex;c!==null&&c>0&&s.setState({lightboxIndex:c-1});return}if(g==="lightbox-next"){let c=s.getState(),m=S(c).length;c.lightboxIndex!==null&&c.lightboxIndex<m-1&&s.setState({lightboxIndex:c.lightboxIndex+1});return}}),document.addEventListener("keydown",i=>{let n=s.getState();if(n.lightboxIndex!==null){if(i.key==="Escape")y=n.lightboxReturnIndex,s.setState({lightboxIndex:null,lightboxReturnIndex:null});else if(i.key==="ArrowLeft")n.lightboxIndex>0&&s.setState({lightboxIndex:n.lightboxIndex-1});else if(i.key==="ArrowRight"){let g=S(n).length;n.lightboxIndex<g-1&&s.setState({lightboxIndex:n.lightboxIndex+1})}}}),v.addEventListener("change",async i=>{let n=i.target;if(n.dataset.action==="set-sort"){let g=n.value;s.setState({sort:g}),await w()}}),v.addEventListener("submit",async i=>{var p,T,W;let n=i.target;if(n.dataset.action!=="submit-write")return;i.preventDefault();let g=s.getState();if(g.writeRating<1){s.setState({writeError:"Please select a star rating."});return}let c=new FormData(n),m={};for(let h of g.attributeDefs){let k=c.get(`attr__${h.key}`);typeof k=="string"&&k&&(m[h.key]=k)}s.setState({writeSubmitting:!0,writeError:null});try{await U(r,{productSlug:a,customerName:String((p=c.get("customerName"))!=null?p:""),rating:g.writeRating,title:String((T=c.get("title"))!=null?T:"")||void 0,body:String((W=c.get("body"))!=null?W:""),attributes:m}),s.setState({writeSubmitting:!1,writeSuccess:!0})}catch(h){s.setState({writeSubmitting:!1,writeError:h instanceof Error?h.message:"Something went wrong."})}}),F(),nt()}function I(t){t.querySelector(".rvos-widget")||st(t)}function gt(t){if(t){I(t);return}document.querySelectorAll("[data-reviewos]").forEach(I)}function ot(){document.querySelectorAll("[data-reviewos]:not([data-reviewos-manual])").forEach(I)}window.ReviewOS={mount:gt};if(window.ReviewOSQueue){let t=window.ReviewOSQueue;window.ReviewOSQueue=[],t.forEach(e=>e())}function it(){document.readyState==="complete"?ot():window.addEventListener("load",()=>ot(),{once:!0})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",it):it();})();
