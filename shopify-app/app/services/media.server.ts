// Review media validation + pending-upload tracking. The validation
// functions are pure, no I/O, so they're portable (kept in sync manually
// with reviewos/app/services/media.server.ts) and trivially unit-testable.
// recordPendingReviewMedia below is shopify-app only (the demo has no
// PendingReviewMedia table / orphan-cleanup concept).
...
export type RecordPendingReviewMediaArgs = {
  storageKey: string;
  url: string;
  mimeType: string;
  sizeBytes: number;
...
export async function recordPendingReviewMedia(shop: string, args: RecordPendingReviewMediaArgs) {
  return prisma.pendingReviewMedia.create({ data: { shop, ...args } });
}
